...
int dooms::GameEngineEntryPoint(int argc, char* argv[])
{
	{
		{
		...
		std::cout << "Remained DObject Count ( Before DestroyAllDObjects ) : " << dooms::DObjectManager::GetDObjectCount() << std::endl;
	...
	std::cout << "Remained DObject Count ( After DestroyAllDObjects )  : " << dooms::DObjectManager::GetDObjectCount() << std::endl;
	std::fflush(stdout);
