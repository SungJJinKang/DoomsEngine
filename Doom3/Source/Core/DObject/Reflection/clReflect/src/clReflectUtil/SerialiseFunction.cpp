...
CLCPP_API bool clutl::BuildParameterObjectCache_JSON(clutl::ParameterObjectCache& poc, const clcpp::Function* function,
                                                     clutl::ReadBuffer& parameter_source, unsigned int transient_flags)
{
    ...
    for (unsigned i = 0; i < nb_fields; i++)
    {
        ...
        JSONError error = LoadJSON(ctx, param_object, field, transient_flags);
