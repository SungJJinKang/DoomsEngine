...
void CubemapTexture::TexImage2D(INT32 level, const void* data) const noexcept
{
	...
	for (UINT32 i = 0; i < 6; i++)
	{
		...
		GraphicsAPI::Define2DTextureStorageRequirement(mBindTarget, level, mInternalFormat, mWidth, mHeight);
		GraphicsAPI::UploadPixelsTo2DTexture(static_cast<GraphicsAPI::eTargetTexture>(GraphicsAPI::eTargetTexture::TARGET_TEXTURE_TEXTURE_CUBE_MAP_POSITIVE_X + i), level, 0, 0, mWidth, mHeight, mDataFormat, mDataType, data);
...
void CubemapTexture::TexImage2D(INT32 face, INT32 level, const void* data) const noexcept
{
	...
	GraphicsAPI::Define2DTextureStorageRequirement(mBindTarget, level, mInternalFormat, mWidth, mHeight);
	GraphicsAPI::UploadPixelsTo2DTexture(static_cast<GraphicsAPI::eTargetTexture>(GraphicsAPI::eTargetTexture::TARGET_TEXTURE_TEXTURE_CUBE_MAP_POSITIVE_X + face), level, 0, 0, mWidth, mHeight, mDataFormat, mDataType, data);
